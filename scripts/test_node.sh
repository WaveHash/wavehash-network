#!/bin/bash
# Run this script to quickly install, setup, and run the current version of wavehash without docker.
#
# Example:
# CHAIN_ID="local-1" HOME_DIR="~/.wavehash1/" TIMEOUT_COMMIT="500ms" CLEAN=true sh scripts/test_node.sh
# CHAIN_ID="local-2" HOME_DIR="~/.wavehash2/" CLEAN=true RPC=36657 REST=2317 PROFF=6061 P2P=36656 GRPC=8090 GRPC_WEB=8091 ROSETTA=8081 TIMEOUT_COMMIT="500ms" sh scripts/test_node.sh
#
# To use unoptomized wasm files up to ~5mb, add: MAX_WASM_SIZE=5000000


export KEY="wavehash1"
export CHAIN_ID=${CHAIN_ID:-"local-1"}
export MONIKER="localwavehash"
export KEYALGO="secp256k1"
export KEYRING=${KEYRING:-"test"}
export HOME_DIR=$(eval echo "${HOME_DIR:-"~/.wavehash/"}")

export BINARY=${BINARY:-wavehashd}
...
wavehashd config keyring-backend $KEYRING
wavehashd config chain-id $CHAIN_ID
...
command -v wavehashd > /dev/null 2>&1 || { echo >&2 "wavehashd command not found. Ensure this is setup / properly installed in your GOPATH."; exit 1; }
...
from_scratch () {
  ...
  # GlobalFee
  update_test_genesis '.app_state["globalfee"]["params"]["minimum_gas_prices"]=[{"amount":"0.002500000000000000","denom":"uwaha"}]'

  update_test_genesis '.app_state["staking"]["params"]["bond_denom"]="uwaha"'  
  update_test_genesis '.app_state["bank"]["params"]["send_enabled"]=[{"denom": "uwaha","enabled": true}]'
  # update_test_genesis '.app_state["staking"]["params"]["min_commission_rate"]="0.100000000000000000"' # sdk 46 only   

  update_test_genesis '.app_state["mint"]["params"]["mint_denom"]="uwaha"'  
  update_test_genesis '.app_state["gov"]["deposit_params"]["min_deposit"]=[{"denom": "uwaha","amount": "1000000"}]'
  update_test_genesis '.app_state["crisis"]["constant_fee"]={"denom": "uwaha","amount": "1000"}'  

  update_test_genesis '.app_state["tokenfactory"]["params"]["denom_creation_fee"]=[{"denom":"uwaha","amount":"100"}]'

  update_test_genesis '.app_state["feeshare"]["params"]["allowed_denoms"]=["uwaha"]'

  # Allocate genesis accounts
  BINARY add-genesis-account $KEY 10000000uwaha,1000utest --keyring-backend $KEYRING
  BINARY add-genesis-account feeacc 1000000uwaha,1000utest --keyring-backend $KEYRING

  BINARY gentx $KEY 1000000uwaha --keyring-backend $KEYRING --chain-id $CHAIN_ID
...
BINARY start --pruning=nothing  --minimum-gas-prices=0uwaha --rpc.laddr="tcp://0.0.0.0:$RPC"