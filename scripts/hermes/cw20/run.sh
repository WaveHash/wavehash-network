...
WAVEHASHD_NODE='http://localhost:26657/'
WAVEHASHD_NODE2='http://localhost:36657/' # receiving chain
# globalfee will break this in the future
export KEY_ADDR="wavehash1hj5fveer5cjtn4wd6wstzugjfdxzl0xps73ftl"

TX_FLAGS="--gas-prices="0.03uwaha" --gas 5000000 -y -b block --node http://localhost:26657 --output json --keyring-backend test --chain-id local-1"
export WAVEHASHD_COMMAND_ARGS="$TX_FLAGS --from wavehash1"

TX_FLAGS2="--gas-prices="0.03uwaha" --gas 5000000 -y -b block --node $WAVEHASHD_NODE2 --output json --keyring-backend test --chain-id local-2"
export WAVEHASHD_COMMAND_ARGS2="$TX_FLAGS2 --from wavehash1"
...
function upload_cw20 {
    ...
    TX=$(wavehashd tx wasm store ./scripts/hermes/cw20/cw20_base.wasm $WAVEHASHD_COMMAND_ARGS | jq -r '.txhash') && echo "$TX"
    CW_CODE_ID=$(wavehashd q tx $TX --output json --node http://localhost:26657 | jq -r '.logs[0].events[] | select(.type == "store_code").attributes[] | select(.key == "code_id").value') && echo "Code Id: $CW_CODE_ID"
    ...
    TX_UPLOAD=$(wavehashd tx wasm instantiate "$CW_CODE_ID" $INIT_JSON --label "e2e-$TYPE" $WAVEHASHD_COMMAND_ARGS --admin $KEY_ADDR | jq -r '.txhash') && echo $TX_UPLOAD
    export CW20_CONTRACT=$(wavehashd query tx $TX_UPLOAD --output json | jq -r '.logs[0].events[0].attributes[0].value') && echo "CW20_CONTRACT: $CW20_CONTRACT"        
}
...
function upload_cw20_ics20 {
    ...
    echo "Storing $TYPE contract..."
    TX=$(wavehashd tx wasm store ./scripts/hermes/cw20/cw20_ics20.wasm $WAVEHASHD_COMMAND_ARGS | jq -r '.txhash') && echo "$TX"
    CW_CODE_ID=$(wavehashd q tx $TX --output json --node http://localhost:26657 | jq -r '.logs[0].events[] | select(.type == "store_code").attributes[] | select(.key == "code_id").value') && echo "Code Id: $CW_CODE_ID"
    ...
    TX_UPLOAD=$(wavehashd tx wasm instantiate "$CW_CODE_ID" $INIT_JSON --label "e2e-$TYPE" $WAVEHASHD_COMMAND_ARGS --admin $KEY_ADDR | jq -r '.txhash') && echo $TX_UPLOAD
    export ICS20_CONTRACT=$(wavehashd query tx $TX_UPLOAD --output json | jq -r '.logs[0].events[0].attributes[0].value') && echo "ICS20_CONTRACT: $ICS20_CONTRACT"          
}
