...
func TestBasicWavehashUpgrade(t *testing.T) {
	repo, version := GetDockerImageInfo()
	// This is a patch upgrade that also includes the tokenfactory changes required for ICS999
	upgradeName := "v6"
	CosmosChainUpgradeTest(t, "wavehash", "v5.0.0", version, repo, upgradeName)
}
	...
	cf := interchaintest.NewBuiltinChainFactory(zaptest.NewLogger(t), []*interchaintest.ChainSpec{
		{
			...
			ChainConfig: ibc.ChainConfig{
				...
				Images: []ibc.DockerImage{
					{
						Repository: WavehashE2ERepo,
						Version:    initialVersion,
						UidGid:     WavehashImage.UidGid,
					},
				},
			},
		},
