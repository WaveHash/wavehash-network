...
import (
	...
	apphelpers "github.com/WaveHashProtocol/wavehash/v6/app/helpers"
	appparams "github.com/WaveHashProtocol/wavehash/v6/app/params"
...
// SetupWithGenesisValSet initializes a new wavehashApp with a validator set and genesis accounts
// that also act as delegators. For simplicity, each validator is bonded with a delegation
// of one consensus engine unit in the default token of the WavehashApp from first genesis
// account. A Nop logger is set in WavehashApp.
func SetupWithGenesisValSet(t *testing.T, valSet *tmtypes.ValidatorSet, genAccs []authtypes.GenesisAccount, balances ...banktypes.Balance) *App {
	...
	wavehashApp, genesisState := setup(true, 5)
	genesisState = genesisStateWithValSet(t, wavehashApp, genesisState, valSet, genAccs, balances...)

	stateBytes, err := json.MarshalIndent(genesisState, "", " ")
	require.NoError(t, err)

	// init chain will set the validator set and initialize the genesis accounts
	wavehashApp.InitChain(
	...
	wavehashApp.Commit()
	wavehashApp.BeginBlock(abci.RequestBeginBlock{Header: tmproto.Header{
		Height:             wavehashApp.LastBlockHeight() + 1,
		AppHash:            wavehashApp.LastCommitID().Hash,
