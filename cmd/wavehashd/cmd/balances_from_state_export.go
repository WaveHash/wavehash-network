...
Example:
	wavehashd export-derive-balances ../genesis.json ../snapshot.json
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Produce the map of address to total wavehash balance, both staked and UnbondingStake
			...
			for _, unbonding := range stakingGenesis.UnbondingDelegations {
				...
				unbondingWavehashs := sdk.NewInt(0)
				for _, entry := range unbonding.Entries {
					unbondingWavehashs = unbondingWavehashs.Add(entry.Balance)
				}

				acc.UnbondingStake = acc.UnbondingStake.Add(unbondingWavehashs)
			...
			for _, delegation := range stakingGenesis.Delegations {
				...
				stakedWavehash := delegation.Shares.MulInt(val.Tokens).Quo(val.DelegatorShares).RoundInt()

				acc.Staked = acc.Staked.Add(stakedWavehash)
...
Example:
	wavehashd staked-to-csv ../balances.json ../airdrop.csv
`,
		Args: cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx := client.GetClientContextFromCmd(cmd)
